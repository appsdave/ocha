...
4. Each builder task runs in its own **isolated git worktree** on its own **unique branch**.
5. A **reviewer agent** checks each builder result.
6. The result is pushed to a **fresh PR** for that branch.
...
- `src/lib/lead.js` — task planning and unique branch naming
...
2. **Lead**
   ...
   - plans subtasks
   - creates descriptive task branches with unique timestamped suffixes
...
5. **Push / PR**
   - each task is pushed from its own branch
   - each task/agent should end on a unique branch and open a fresh PR
...
Every task is supposed to be isolated in its own worktree and branch so that:
...
So the TUI is really visualizing a queue of isolated branches, not just a queue of logs.
...
That is the main concept: **one operator console for many isolated agent branches**.
